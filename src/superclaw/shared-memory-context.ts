...
export async function injectRelevantMemories(
  ...
  try {
    ...
    log.debug(
      `Injected ${memories.length} shared memories into context (${estimatedTokens} tokens)`,
    );
...
export async function getRecentLearnings(
  ...
  const { maxMemories = 3, minImportance = 0.5, maxAgeHours = 24, format = "markdown" } = options;
...
export async function storeDecision(
  ...
  try {
    ...
    const content = reasoning ? `Decision: ${decision}\n\nReasoning: ${reasoning}` : decision;
