...
export class Convoy extends EventEmitter {
  ...
  async start(): Promise<ConvoyResult> {
    ...
    try {
      ...
      // @ts-expect-error - Post-Merge Reconciliation
      this.addEvent('failed', `Convoy failed: ${error instanceof Error ? (error as Error).message : String(error)}`);
    ...
    for (const gate of this.config.qualityGates) {
      try {
        ...
        const result = {
          ...
          message: `Gate execution failed: ${error instanceof Error ? (error as Error).message : String(error)}`,
        };
