...
import { injectRelevantMemories, storeLesson, storeDecision, getRecentLearnings } from './shared-memory-context.js';
...
async function demo() {
  ...
  try {
    ...
    await sharedMemory.store({
      agentId: 'swarm-001',
      content: 'Decided to use SQLite for shared memory instead of in-memory storage for persistence across restarts',
      type: 'decision',
    ...
    for (const memory of codeRelevantMemories) {
      console.log(`   - [${memory.type}] ${memory.content.slice(0, 50)}... (importance: ${memory.importance})`);
    }
    console.log();
    ...
    console.log(`   Generated context with ${context.memoriesIncluded} memories (${context.estimatedTokens} tokens):`);
