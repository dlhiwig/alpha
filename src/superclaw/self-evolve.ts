...
export class SelfEvolver {
   ...
   */
  detectOpportunities(
    oracleInsights: { pattern: string; frequency: number; recommendation: string; confidence: number }[],
    oracleMistakes: { pattern: string; rootCause: string; correction: string; severity: RiskLevel }[],
  ...
  addOpportunity(
    description: string,
    opts: { priority?: RiskLevel; impact?: RiskLevel; suggestedFix?: string; filePaths?: string[] } = {},
  ): EvolutionOpportunity {
    const opp: EvolutionOpportunity = {
      id: this.generateId("opp"),
      source: "manual",
  ...
  private async createPR(plan: EvolutionPlan): Promise<void> {
    ...
    try {
      ...
      const prBody = [
        ...
        "### Changes",
        ...plan.patches.map(
          (p) => `- \`${p.file}\`: ${p.action}${p.diff ? ` — ${p.diff}` : ""}`,
        ),
        "",
        "---",
