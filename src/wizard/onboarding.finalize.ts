...
export async function finalizeOnboardingWizard(
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        `View token: ${formatCliCommand("alpha config get gateway.auth.token")}`,
        `Generate token: ${formatCliCommand("alpha doctor --generate-gateway-token")}`,
        "Web UI keeps dashboard URL tokens in memory for the current tab and strips them from the URL after load.",
        `Open the dashboard anytime: ${formatCliCommand("alpha dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("alpha dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://alpha-docs.vercel.app/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://alpha-docs.vercel.app/security",
    "Security",
  );
    ...
    if (webSearchEnabled !== false && hasKey) {
      ...
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("alpha configure --section web")}`,
          "",
          `Get your key at: ${entry?.signupUrl ?? "https://alpha-docs.vercel.app/tools/web"}`,
          "Docs: https://alpha-docs.vercel.app/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("alpha configure --section web")}`,
          "",
          "Docs: https://alpha-docs.vercel.app/tools/web",
        ].join("\n"),
        "Web search",
