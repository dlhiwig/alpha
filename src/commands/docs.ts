...
const SEARCH_TOOL = "https://alpha-docs.vercel.app/mcp.SearchOpenClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "alpha-docs.vercel.app");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('alpha docs "your query"')}`);
    } else {
      runtime.log("Docs: https://alpha-docs.vercel.app/");
      runtime.log(`Search: ${formatCliCommand('alpha docs "your query"')}`);
    }
    return;
  }
