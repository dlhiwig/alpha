...
export async function noteSecurityWarnings(cfg: OpenClawConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("alpha security audit --deep")}`;
  ...
  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      ...
      `  Check local policy with: ${formatCliCommand("alpha approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://alpha-docs.vercel.app/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("alpha configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("alpha config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("alpha doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("alpha config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('alpha config set session.dmScope "per-channel-peer"') +
