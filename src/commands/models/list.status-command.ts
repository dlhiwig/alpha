...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("alpha models auth setup-token")}\` or \`${formatCliCommand("alpha configure")}\`.`
          : `Run \`${formatCliCommand("alpha configure")}\` or set an API key env var.`;
