...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Alpha and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["alpha update", "Update a source checkout (git)"],
        ["alpha update --channel beta", "Switch to beta channel (git + npm)"],
        ["alpha update --channel dev", "Switch to dev channel (git + npm)"],
        ["alpha update --tag beta", "One-off update to a dist-tag or version"],
        ["alpha update --dry-run", "Preview actions without changing anything"],
        ["alpha update --no-restart", "Update without restarting the service"],
        ["alpha update --json", "Output result as JSON"],
        ["alpha update --yes", "Non-interactive (accept downgrade prompts)"],
        ["alpha update wizard", "Interactive update wizard"],
        ["alpha --update", "Shorthand for openclaw update"],
      ] as const;
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "alpha-docs.vercel.app/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["alpha update status", "Show channel + version status."],
          ["alpha update status --json", "JSON output."],
          ["alpha update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "alpha-docs.vercel.app/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
