...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "alpha-docs.vercel.app/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Alpha hooks")
    ...
    .option("--hook-url <url>", "Alpha hook URL")
    .option("--hook-token <token>", "Alpha hook token")
